...
(define (moves-row-none row)
  (let L ([row row]
    ...
    (if (null? row)
        ...
        (let ([value (car row)])
          (if (nonzero? value)
              (L (cdr row) (add1 i) (cons (list value i i) result))
              (L (cdr row) (add1 i) result)))))
  #;(for/list ([value row]
             [i (in-naturals)]
             #:when (nonzero? value))
    (list value i i)))
...
(define (moves-grid-action grid action)
  (let ([n (length (first grid))])
    (apply append
           (map
            (lambda (row i) (add-row-coord i (action row)))
            grid (build-list n (lambda (x) x)))
           #;(for/list ([row grid]
                        [i (in-range n)])
...
(define (moves-grid-rotate grid)
  (let ([n (length (first grid))])
    (map (lambda (item)
         ...
         (moves-grid-action grid moves-row-none))
...
;;
(define (square sz fill color)
  (rectangle sz sz fill color))
             ...
             (foldl
              (lambda (j im)
                (place-tile/ij (make-tile (square/ij state i j))
                           ...
                           im))
              acc
              (build-list *side* (lambda (x) x))))
           (square *grid-side* 'solid *grid-color*)
           (build-list *side* (lambda (x) x)))
...
(define (number->time-string s)
  ...
  (define (xx n)
    (cond [(<= n 0) "00"]
          [(<= n 9) #;(format "0~a" n) (string-append "0" (number->string n))]
          [else (number->string (remainder n 60))]))
  (if (>= s 3600)
      (string-append (number->string hrs) ":" (xx mins) ":" (xx secs))
      (string-append (number->string mins) ":" (xx secs))
...
(define (show-world w)
  ...
  (let* ([board (if (null? frames)
                    ...
                    ((first frames)))]
         [score-text (text #;(format "Score: ~a" score) (string-append "Score: " (number->string score)) 16 'dimgray)]
         [seconds ((if *time-limit* time-remaining time-elapsed) start-time)]
         [time-text (text #;(format "Time: ~a" 
                                    (number->time-string seconds))
                          (string-append "Time: " (number->time-string seconds))
                          16
